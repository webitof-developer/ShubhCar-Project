...
import { Card, CardBody, Col, Row, Spinner, Table, Button, Form, Modal, CardHeader, CardTitle } from 'react-bootstrap'
    ...
    const handleSave = async () => {
        ...
        try {
            const url = editingBrand
                ? `${API_BASE_URL}/vehicle-brands/${editingBrand._id}`
                : `${API_BASE_URL}/vehicle-brands`
    ...
    const confirmDelete = async () => {
        ...
        try {
            const response = await fetch(
                `${API_BASE_URL}/vehicle-brands/${deletingId}`,
                {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${session.accessToken}`,
        ...
        <>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle as={'h4'} className="d-flex align-items-center gap-1">
                                <IconifyIcon icon="mdi:car-brake-parking" className="text-primary fs-20" />
                                Vehicle Brands
                            </CardTitle>
                        </CardHeader>
                        <CardBody>
                            <div className="d-flex justify-content-end mb-3">
                                <Button variant="primary" onClick={() => handleOpenModal()}>Add Vehicle Brand</Button>
                            </div>
                            <div className="table-responsive">
                                <Table hover responsive className="table-nowrap mb-0 align-middle">
                                    ...
                                    <tbody>
                                        ...
                                        {brands.length === 0 && <tr><td colSpan="5" className="text-center">No vehicle brands found</td></tr>}
                                    </tbody>
                                </Table>
                            </div>
                        </CardBody>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                <Modal.Header closeButton>
                    <Modal.Title>{editingBrand ? 'Edit' : 'Add'} Vehicle Brand</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form>
