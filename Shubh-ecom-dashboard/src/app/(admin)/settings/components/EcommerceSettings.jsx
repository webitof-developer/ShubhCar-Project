    ...
    const fetchSettings = async () => {
      ...
      try {
        ...
        setFormData((prev) => ({
          ...
          billing_country: data.billing_country || INDIA_COUNTRY.code,
      ...
      await settingsAPI.update(
        {
          ...
          billing_country: formData.billing_country || INDIA_COUNTRY.code,
                  ...
                  <Row className="mt-3">
                    ...
                    <Col lg={6}>
                      <div className="mb-3">
                        ...
                        <input id="store_owner" className="form-control" value={formData.store_owner} onChange={(e) => setFormData((prev) => ({ ...prev, store_owner: e.target.value }))} />
                      </div>
                    </Col>
                    <Col lg={6}>
                      <div className="mb-3">
                    ...
                    <Col lg={12}><h6 className="text-muted text-uppercase mt-2">Billing Address</h6></Col>
                    <Col lg={6}>
                      <div className="mb-3">
                        <label htmlFor="billing_name" className="form-label">Billing Contact Name</label>
                        <input id="billing_name" className="form-control" value={formData.billing_name} onChange={(e) => setFormData((prev) => ({ ...prev, billing_name: e.target.value }))} />
                      </div>
                    </Col>
                    <Col lg={6}>
                      <div className="mb-3">
                        <label htmlFor="billing_phone" className="form-label">Billing Phone</label>
                        <input id="billing_phone" className="form-control" value={formData.billing_phone} onChange={(e) => setFormData((prev) => ({ ...prev, billing_phone: e.target.value }))} />
                      </div>
                    </Col>
                    <Col lg={12}>
                      <div className="mb-3">
                        <label htmlFor="billing_address" className="form-label">Billing Address</label>
                        <textarea id="billing_address" className="form-control" rows={3} value={formData.billing_address} onChange={(e) => setFormData((prev) => ({ ...prev, billing_address: e.target.value }))} />
                      </div>
                    </Col>
                    <Col lg={4}>
                      <div className="mb-3">
                        <label htmlFor="billing_city" className="form-label">City</label>
                        <input id="billing_city" className="form-control" value={formData.billing_city} onChange={(e) => setFormData((prev) => ({ ...prev, billing_city: e.target.value }))} />
                      </div>
                    </Col>
                    <Col lg={4}>
                      <div className="mb-3">
                    ...
                    <Col lg={4}>
                      <div className="mb-3">
                        ...
                        <input id="billing_zip" className="form-control" value={formData.billing_zip} onChange={(e) => setFormData((prev) => ({ ...prev, billing_zip: e.target.value }))} />
                      </div>
                    </Col>
                    <Col lg={6}>
                      <div className="mb-3">
                        <label htmlFor="billing_country" className="form-label">Country</label>
                        <Form.Select id="billing_country" value={formData.billing_country} onChange={(e) => setFormData((prev) => ({ ...prev, billing_country: e.target.value }))}>
                          <option value={INDIA_COUNTRY.code}>{INDIA_COUNTRY.name}</option>
                        </Form.Select>
                      </div>
                    </Col>
                      ...
                      <div className="mb-3">
                        ...
                        {formData.site_logo_dark ? (
                          <div className="position-relative d-inline-block">
                            <img src={resolveMediaUrl(formData.site_logo_dark)} alt="Dark Logo" className="rounded border bg-white" style={{ width: 140, height: 60, objectFit: 'contain' }} />
                            <Button type="button" variant="light" className="position-absolute top-0 start-100 translate-middle p-0 rounded-circle border" style={{ width: 26, height: 26 }} onClick={() => setFormData((prev) => ({ ...prev, site_logo_dark: '' }))} title="Remove logo">
                              <IconifyIcon icon="mdi:close" width={16} height={16} />
                            </Button>
                          </div>
                        ) : (
                          <div className="d-flex flex-column gap-2">
                            <DropzoneFormInput text="Drag & drop dark logo here, or click to upload" textClassName="fs-6" className="py-4" iconProps={{ icon: 'bx:cloud-upload', width: 28, height: 28 }} showPreview={false} maxFiles={1} onFileUpload={(files) => uploadLogo(files, 'site_logo_dark')} />
                            <Button variant="outline-primary" size="sm" onClick={() => { setMediaTarget('site_logo_dark'); setShowMediaPicker(true) }}>Choose from Media Library</Button>
                          </div>
                        )}
                      </div>
                    </Col>
                    <Col lg={6}>
                      <div className="mb-3">
                        ...
                        {formData.site_logo_light ? (
                          <div className="position-relative d-inline-block">
                            <img src={resolveMediaUrl(formData.site_logo_light)} alt="Light Logo" className="rounded border bg-dark" style={{ width: 140, height: 60, objectFit: 'contain' }} />
                            <Button type="button" variant="light" className="position-absolute top-0 start-100 translate-middle p-0 rounded-circle border" style={{ width: 26, height: 26 }} onClick={() => setFormData((prev) => ({ ...prev, site_logo_light: '' }))} title="Remove logo">
                              <IconifyIcon icon="mdi:close" width={16} height={16} />
                            </Button>
                          </div>
                        ) : (
                          <div className="d-flex flex-column gap-2">
                            <DropzoneFormInput text="Drag & drop light logo here, or click to upload" textClassName="fs-6" className="py-4" iconProps={{ icon: 'bx:cloud-upload', width: 28, height: 28 }} showPreview={false} maxFiles={1} onFileUpload={(files) => uploadLogo(files, 'site_logo_light')} />
                            <Button variant="outline-primary" size="sm" onClick={() => { setMediaTarget('site_logo_light'); setShowMediaPicker(true) }}>Choose from Media Library</Button>
                          </div>
                        )}
                      </div>
                    </Col>
                    <Col lg={6}>
                      <div className="mb-3">
                        ...
                        {formData.site_favicon ? (
                          <div className="position-relative d-inline-block">
                            <img src={resolveMediaUrl(formData.site_favicon)} alt="Favicon" className="rounded border bg-white" style={{ width: 64, height: 64, objectFit: 'contain' }} />
                            <Button type="button" variant="light" className="position-absolute top-0 start-100 translate-middle p-0 rounded-circle border" style={{ width: 22, height: 22 }} onClick={() => setFormData((prev) => ({ ...prev, site_favicon: '' }))} title="Remove favicon">
                              <IconifyIcon icon="mdi:close" width={14} height={14} />
                            </Button>
                          </div>
                        ) : (
                          <div className="d-flex flex-column gap-2">
                            <DropzoneFormInput text="Drag & drop favicon here, or click to upload" textClassName="fs-6" className="py-4" iconProps={{ icon: 'bx:cloud-upload', width: 28, height: 28 }} showPreview={false} maxFiles={1} onFileUpload={(files) => uploadLogo(files, 'site_favicon')} />
                            <Button variant="outline-primary" size="sm" onClick={() => { setMediaTarget('site_favicon'); setShowMediaPicker(true) }}>Choose from Media Library</Button>
                          </div>
                        )}
