...
export default function OrdersError({ error, reset }) {
  useEffect(() => {
    logger.error('Orders route error boundary:', error);
  }, [error]);
    ...
    <Container className="d-flex flex-column justify-content-center align-items-center min-vh-100 py-5">
      <Card className="text-center shadow-sm border-0" style={{ maxWidth: '560px', width: '100%' }}>
        <Card.Body className="p-5">
          <IconifyIcon icon="tabler:package-off" className="text-danger mb-4" style={{ fontSize: '64px' }} />
          <h3 className="mb-3 fw-bold">Orders page failed to load</h3>
          <p className="text-muted mb-4">An unexpected error occurred in order management.</p>
