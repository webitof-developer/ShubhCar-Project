...
async function check() {
  try {
    ...
    console.error(`Total categories: ${all.length}`);

    const roots = await Category.find({ parentId: null });
    console.error(`Explicit roots (parentId: null): ${roots.length}`);
    
    roots.forEach(r => {
        console.error(`ROOT: ${r.name} (${r._id}) isActive:${r.isActive}`);
    });

    const activeRoots = await Category.find({ parentId: null, isActive: true });
    console.error(`Active roots: ${activeRoots.length}`);
    
    // Check for "Filter" specifically if it was seen before
    const filter = await Category.findOne({ name: 'Filter' });
    if(filter) console.error('Filter cat:', filter);
