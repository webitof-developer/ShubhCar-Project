  ...
  async adminImportCustomers(actor, file) {
    ...
    for (let rowNumber = 2; rowNumber <= sheet.rowCount; rowNumber += 1) {
      ...
      try {
        ...
        const email = toCellString(
          row.getCell(headerMap.email)?.value,
        ).toLowerCase();
        const phone = toCellString(row.getCell(headerMap.phone)?.value);
        const customerType = normalizeEnum(
          toCellString(row.getCell(headerMap.customer_type)?.value),
        );
        const status = normalizeEnum(
          toCellString(row.getCell(headerMap.status)?.value),
        );
        let verificationStatus = normalizeEnum(
          toCellString(row.getCell(headerMap.verification_status)?.value),
        );
        ...
        if (verificationStatus === 'notrequired')
          verificationStatus = 'not_required';
        if (
          !['approved', 'pending', 'not_required'].includes(verificationStatus)
        ) {
