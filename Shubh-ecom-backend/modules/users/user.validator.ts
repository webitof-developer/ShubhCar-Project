...
const registerUserSchema = Joi.object({
  ...
  role: Joi.string().valid(ROLES.CUSTOMER, ROLES.ADMIN).default(ROLES.CUSTOMER),
...
const adminCreateSchema = Joi.object({
  ...
  password: Joi.string().min(6).required(),
  role: Joi.string()
    .valid(ROLES.CUSTOMER, ROLES.ADMIN, ROLES.SALESMAN)
    .optional(),
  roleId: Joi.string().trim().allow('').optional(),
  customerType: Joi.string().valid('retail', 'wholesale').default('retail'),
...
const adminUpdateSchema = Joi.object({
  ...
  phone: Joi.string()
    ...
    .allow('', null)
    .optional(),
  password: Joi.string().min(6).optional(), // Allow admin to set/change user password
