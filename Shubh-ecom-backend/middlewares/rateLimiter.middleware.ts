...
const buildRateLimitHandler = (message: string) => (req: any, res: any) => {
...
const ipKey = (req: any) => ipKeyGenerator(req);
...
exports.apiLimiter = rateLimit({
  ...
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 1000,
  skip: (req: any) => req.originalUrl?.startsWith('/api/v1/payments/webhook'),
  handler: buildRateLimitHandler('Too many requests, please try again later'),
});
...
exports.adminLimiter = rateLimit({
  ...
  windowMs: 15 * 60 * 1000,
  max: 200,
  keyGenerator: (req: any) => (req.user?.id ? `admin:${req.user.id}` : ipKey(req)),
  handler: buildRateLimitHandler('Too many requests, please try again later'),
});
...
exports.paymentLimiter = rateLimit({
  ...
  max: 30,
  keyGenerator: (req: any) => (req.user?.id ? `payment:${req.user.id}` : ipKey(req)),
  handler: buildRateLimitHandler('Too many requests, please try again later'),
});
