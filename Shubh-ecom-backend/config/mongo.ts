...
const connectMongo = async (): Promise<void> => {
  ...
  mongoose.plugin((schema: any) => {
    ...
    trackedOps.forEach((op: string) => {
      schema.pre(op, function trackStart(this: any) {
        this._metricsStart = Date.now();
      });
      schema.post(op, function logDuration(this: any) {
    ...
    schema.pre('aggregate', function trackAggregateStart(this: any) {
      this._metricsStart = Date.now();
    });
    schema.post('aggregate', function logAggregateDuration(this: any) {
      const duration = Date.now() - (this._metricsStart || Date.now());
      if (duration >= slowQueryThreshold) {
  ...
  const mongoUri = (useReplica ? env.MONGO_REPLICA_URI : env.MONGO_URI) as string;

  mongoose.connection.on('connected', () => {
    logger.info('dYY› MongoDB connected');
  });

  mongoose.connection.on('error', (err: unknown) => logger.error('Mongo error', err));
  ...
  const connectWithRetry = async (retries = 5): Promise<void> => {
    try {
      ...
      await mongoose.connect(mongoUri, {
        ...
        readPreference: 'primary', // Must be 'primary' for MongoDB transactions (replica set requirement)
      });
  ...
  const ensureReplicaIfRequired = (): void => {
